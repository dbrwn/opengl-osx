...
- (void) initializeVertexArrayObjects
{
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferObject); // does this set up for the next attribute pointer config?
    ...
    // use the same buffer object previously bound to GL_BUFFER_DATA
    glEnableVertexAttribArray(0);
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)posDataOffset);
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 0, (void*)colorDataOffset);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferObject);  // either this one or the one above is unimportant?
...
- (void) prepareOpenGL
{
    ...
    // using the perspective matrix causes the view to strech if it's resized to a non 1:1 aspect ratio
    //    GLKMatrix4 perspective = GLKMatrix4MakePerspective(M_PI_2, 1.0, 1.0, 3.0);
